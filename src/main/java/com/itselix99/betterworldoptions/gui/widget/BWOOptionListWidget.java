...
public class BWOOptionListWidget extends EntryListWidgetButtons implements BWOScreen {
    ...
    public BWOOptionListWidget(Minecraft minecraft, int width, int height, int yStart, int yEnd, int itemHeight, BWOWorldPropertiesStorage bwoWorldPropertiesStorage, Option<?>... options) {
        ...
        if (options != null) {
            for (int var1 = 0; var1 < options.length; var1 += 2) {
                Option<?> option = options[var1];
                Option<?> option2 = var1 < options.length - 1 ? options[var1 + 1] : null;
    ...
    private BWOButtonWidget createWidget(int x, Option<?> option) {
        if (option == null) {
            return null;
        } else {
            int var1 = option.getId();
            BWOButtonWidget button;
            if (option instanceof StringOption) {
                String stringOptionValue = this.bwoWorldPropertiesStorage.getOptionValue(option.getName(), option.getOptionType(), "");
                button = new BWOButtonWidget(var1, x, 0, this.translation.get(option.getDisplayName()) + " " + stringOptionValue, option, this.bwoWorldPropertiesStorage, this);
            } else if (option instanceof BooleanOption booleanOption) {
                boolean booleanOptionValue = this.bwoWorldPropertiesStorage.getOptionValue(option.getName(), option.getOptionType(), false);
                button = new BWOButtonWidget(var1, x, 0, this.translation.get(option.getDisplayName()) + " " + (booleanOptionValue ? this.translation.get("options.on") : this.translation.get("options.off")), option, this.bwoWorldPropertiesStorage, this);

                if (!booleanOption.getWorldTypeValues().isEmpty() && booleanOption.getWorldTypeValues().containsKey(Identifier.of(this.bwoWorldPropertiesStorage.getOptionValue("WorldType", OptionType.GENERAL_OPTION, "")))) {
                    button.active = false;
                }

            } else if (option instanceof IntOption) {
                button = new BWOSliderWidget(var1, x, 0, option.getDisplayName(), option, this.bwoWorldPropertiesStorage, this);
            } else {
                return null;
            }

            WorldType worldType = WorldType.getWorldTypeById(Identifier.of(this.bwoWorldPropertiesStorage.getOptionValue("WorldType", OptionType.GENERAL_OPTION, "")));

            if (option.getCompatibleType().equals("Overworld")) {
                if (worldType.isDimension()) {
                    this.bwoWorldPropertiesStorage.resetGeneralOptionToDefaultValue(option);
                    button.active = false;
                }
            } else if (!option.getCompatibleType().equals("All") && !option.getCompatibleWorldTypes().contains(worldType.getId())) {
                this.bwoWorldPropertiesStorage.resetGeneralOptionToDefaultValue(option);
                button.active = false;
            }
